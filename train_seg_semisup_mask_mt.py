...
def train_seg_semisup_mask_mt(submit_config: job_helper.SubmitConfig, dataset, model, arch, freeze_bn,
    ...
    train_unsup_ds = ds_src.dataset(labels=False, mask=True, xf=False, pair=False,
                                    transforms=seg_transforms.SegTransformCompose(train_transforms),
                                    pipeline_type='cv')
    eval_ds = ds_src.dataset(labels=True, mask=False, xf=False, pair=False,
            ...
            if cons_weight > 0.0:
                for _ in range(unsup_batch_ratio):
                    ...
                    if mask_mix:
                        ...
                        batch_ux_mixed = batch_ux0 * (1 - batch_mix_masks) + batch_ux1 * batch_mix_masks
                        ...
                        batch_cut_masks = mask_generator.torch_masks_from_params(
                            batch_mask_params, batch_ux.shape[2:4], torch_device)
