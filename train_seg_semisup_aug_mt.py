...
@job_helper.job('train_seg_semisup_aug_mt', enumerate_job_names=False)
def train_seg_semisup_aug_mt(submit_config: job_helper.SubmitConfig, dataset, model, arch, freeze_bn,
    ...
    if crop_size is not None:
        if aug_scale_hung:
            train_transforms.append(seg_transforms_cv.SegCVTransformRandomCropScaleHung(crop_size, (aug_offset_range, aug_offset_range), uniform_scale=not aug_scale_non_uniform))
        elif aug_max_scale != 1.0 or aug_rot_mag != 0.0:
            train_transforms.append(seg_transforms_cv.SegCVTransformRandomCropRotateScale(
                crop_size, (aug_offset_range, aug_offset_range), rot_mag=aug_rot_mag, max_scale=aug_max_scale, uniform_scale=not aug_scale_non_uniform,
                constrain_rot_scale=not aug_free_scale_rot))
        else:
            train_transforms.append(seg_transforms_cv.SegCVTransformRandomCrop(crop_size, (aug_offset_range, aug_offset_range)))
    else:
        if aug_scale_hung:
        ...
        for sup_batch in itertools.islice(train_sup_iter, iters_per_epoch):
            ...
            if cons_weight > 0.0:
                for _ in range(unsup_batch_ratio):
                    ...
                    batch_ux0 = unsup_batch['sample0']['image'].to(torch_device)
                    ...
                    grid_tea_to_stu = F.affine_grid(batch_ufx0_to_1, batch_ux0.shape, **affine_align_corners_kw)
                    # Transform teacher predicted logits to student space
                    logits_cons_tea_in_stu = F.grid_sample(logits_cons_tea, grid_tea_to_stu, **affine_align_corners_kw)
                    # Transform mask from teacher to student space and multiply by student space mask
                    mask_tea_in_stu = F.grid_sample(batch_um0, grid_tea_to_stu, **affine_align_corners_kw) * batch_um1
                    ...
                    prob_cons_tea_in_stu = F.grid_sample(prob_cons_tea, grid_tea_to_stu, **affine_align_corners_kw)
                    ...
                    if cons_loss_fn == 'var':
                        ...
                        consistency_loss = F.smooth_l1_loss(logits_cons_stu,
                                                            logits_cons_tea_in_stu, reduce=False)
                        consistency_loss = consistency_loss.sum(dim=1, keepdim=True) / root_n_classes
...
@click.option('--aug_offset_range', type=float, default=16.0)
...
def experiment(job_desc, dataset, model, arch, freeze_bn,
    ...
    train_seg_semisup_aug_mt.submit(**params)
